...
function layoutDFG(dfg) {
  ...
  const boxW = 160, boxH = 50, colGap = 190, rowGap = 110, marginX = 40, marginY = 40;
  const positions = {};
  Object.keys(byLevel).sort((a, b) => a - b).forEach(lvl => {
    byLevel[lvl].forEach((n, i) => {
      positions[n.id] = { x: marginX + i * colGap, y: marginY + Number(lvl) * rowGap, level: Number(lvl) };
    });
  });
  const levels = Object.values(level);
  const maxLevel = levels.length ? Math.max(...levels) : 0;
  const maxSiblings = Math.max(...Object.values(byLevel).map(a => a.length), 1);
  return {
    positions, boxW, boxH, backKeys,
    width: marginX * 2 + (maxSiblings - 1) * colGap + boxW + 20,
    height: marginY * 2 + maxLevel * rowGap + boxH + 40
  };
}
...
function pfOnSliderChange() {
  const actVal = document.getElementById('pfActivitySlider').value;
  const pathVal = document.getElementById('pfPathSlider').value;
  document.getElementById('pfActivityPctLabel').textContent = actVal + '%';
  document.getElementById('pfPathPctLabel').textContent = pathVal + '%';
  document.getElementById('pfActivityRing').style.setProperty('--pct', actVal);
  document.getElementById('pfPathRing').style.setProperty('--pct', pathVal);
  drawProcessFlow();
}

function pfStepSlider(sliderId, delta) {
  const el = document.getElementById(sliderId);
  el.value = Math.max(1, Math.min(100, Number(el.value) + delta));
  pfOnSliderChange();
}

function pfResetSlider(sliderId) {
  document.getElementById(sliderId).value = 100;
  pfOnSliderChange();
}
...
function renderSVG() {
  ...
  const positive = cssVar('--positive'), danger = cssVar('--danger'), accent = cssVar('--accent');
  const panel2 = cssVar('--panel2'), textDim = cssVar('--text-dim'), text = cssVar('--text'), panel = cssVar('--panel');
  ...
  let svg = `<svg width="${svgW}" height="${svgH}" viewBox="0 0 ${layout.width} ${layout.height}">
    <defs><marker id="pfArrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
      <path d="M1 1L9 5L1 9" fill="none" stroke="${textDim}" stroke-width="1.6"/></marker></defs>`;
  ...
  pfLastDFG.edges.forEach(e => {
    ...
    let d, midX, midY;
    if (isBack) {
      // Rework/loop — route out to the right side rather than through the vertical flow.
      const x1 = p1.x + layout.boxW, y1 = p1.y + layout.boxH / 2;
      const x2 = p2.x + layout.boxW, y2 = p2.y + layout.boxH / 2;
      midX = Math.max(p1.x, p2.x) + layout.boxW + 55;
      midY = (y1 + y2) / 2;
      d = `M ${x1} ${y1} C ${midX} ${y1}, ${midX} ${y2}, ${x2} ${y2}`;
    } else {
      // Forward flow — top-to-bottom: bottom-center of source to top-center of target.
      const x1 = p1.x + layout.boxW / 2, y1 = p1.y + layout.boxH;
      const x2 = p2.x + layout.boxW / 2, y2 = p2.y;
      midX = (x1 + x2) / 2;
      midY = (y1 + y2) / 2;
      d = `M ${x1} ${y1} C ${x1} ${y1 + 35}, ${x2} ${y2 - 35}, ${x2} ${y2}`;
    }
    const durText = formatDuration(e.avgDurMs);
    svg += `<path class="pf-edge" data-label="${e.from} \u2192 ${e.to}" data-value="${e.freq}" data-duration="${durText}" d="${d}" fill="none" stroke="${isBack ? danger : accent}" stroke-width="${strokeW}" opacity="0.65" marker-end="url(#pfArrow)" style="cursor:pointer;"/>`;
    if (durText) {
  ...
  pfLastDFG.nodes.forEach(n => {
    ...
    if (!p) return;
    const boxColor = n.isStart ? positive : n.isEnd ? danger : accent;
    svg += `<g class="pf-node" data-label="${n.id}" data-value="${n.freq}" data-repeats="${n.repeats}" style="cursor:grab;">
      <rect x="${p.x}" y="${p.y}" width="${layout.boxW}" height="${layout.boxH}" rx="8" fill="${panel2}" stroke="${boxColor}" stroke-width="2"/>
      <text x="${p.x + layout.boxW / 2}" y="${p.y + 15}" text-anchor="middle" fill="${text}" font-size="12" font-weight="600">${String(n.id).slice(0, 20)}</text>
      <text x="${p.x + layout.boxW / 2}" y="${p.y + 29}" text-anchor="middle" fill="${textDim}" font-size="10">${n.freq} occurrences</text>
      ${n.repeats > 0 ? `<text x="${p.x + layout.boxW / 2}" y="${p.y + 42}" text-anchor="middle" fill="${cssVar('--warning')}" font-size="9">${n.repeats} back-to-back repeats</text>` : ''}
    </g>`;
  });
