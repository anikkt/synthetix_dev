function refreshHomeView() {
  ...
  dashWrap.innerHTML = recentDash.length ? recentDash.map(d => `
    <div class="home-card" onclick="openDashboard(${d.id})">
      <div class="home-card-kind">Dashboard</div>
      <b>${d.name}</b>
      <div class="home-card-meta">${d.widgets.length} widget${d.widgets.length === 1 ? '' : 's'} &middot; edited ${d.edited}</div>
    </div>`).join('') : '<div class="home-empty">No dashboards yet.</div>';
