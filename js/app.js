...
document.getElementById('importInput').addEventListener('change', async (e) => {
  ...
  State.dashboards = (imported.dashboards || []).map(d => {
    delete d.firestoreId; // force re-creation as new docs owned by whoever is importing
    d.ownerUid = currentUser.uid;
    d.sharedWith = {};
    if (!d.sheets) d.sheets = [{ id: 1, name: 'Sheet 1', widgets: d.widgets || [] }];
    d.currentSheetId = d.sheets[0].id;
    return d;
  });
