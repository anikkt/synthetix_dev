...
function selectComponentType(cat, type, supported) {
  const kind = cat === 'Single KPI components' ? 'kpi' : cat === 'Selection components' ? 'selection' : cat === 'Content' ? type : 'chart';
  ...
  else if (kind === 'selection') renderSelectionConfig();
  else if (kind === 'textbox') renderTextboxConfig();
  else if (kind === 'image') renderImageConfig();
}
...
function editWidget(id) {
  ...
  if (!d) return;
  const w = currentSheet(d).widgets.find(x => x.id === id);
  if (!w) return;
  document.getElementById('widgetBuilder').classList.add('open');
  if (w.kind === 'chart') {
    ...
    builderState = { kind: 'textbox', type: 'textbox', supported: true, mode: 'nocode', editingId: id };
    renderTextboxConfig(w);
  } else if (w.kind === 'image') {
    builderState = { kind: 'image', type: 'image', supported: true, mode: 'nocode', editingId: id };
    renderImageConfig(w);
  } else {
...
function createChartWidget() {
  ...
  if (builderState.editingId) {
    const w = sheet.widgets.find(w => w.id === builderState.editingId);
    if (w) Object.assign(w, props);
  } else {
    sheet.widgets.push(newWidgetLayout({ id: Date.now(), kind: 'chart', ...props }, sheet));
  }
...
function createKPIWidget() {
  ...
  if (builderState.editingId) {
    ...
    sheet.widgets.push(newWidgetLayout({ id: Date.now(), kind: 'kpi', ...props }, sheet));
  }
  d.edited = fmtDate(new Date());
  closeWidgetBuilder();
  renderWidgets();
...
function createSelectionWidget() {
  ...
  if (builderState.editingId) {
    ...
    sheet.widgets.push(newWidgetLayout({ id: Date.now(), kind: 'selection', ...props }, sheet));
  }
  d.edited = fmtDate(new Date());
  closeWidgetBuilder();
  renderWidgets();
...
/* ---- Text box (static/markdown-ish note on the dashboard) ---- */
function renderTextboxConfig(existing) {
  const isEdit = !!(existing && builderState.editingId);
  const html = `
    <button class="link" onclick="${isEdit ? 'closeWidgetBuilder()' : 'openWidgetBuilder()'}">&larr; ${isEdit ? 'Close' : 'Back'}</button>
    <div style="margin-top:6px;"><b>${isEdit ? 'Edit: ' : ''}Text box</b></div>
    <div class="row" style="margin-top:12px;"><input type="text" id="tb-title" placeholder="Widget title (optional)" style="flex:1;" value="${esc(existing && existing.title)}"></div>
    <textarea id="tb-content" placeholder="Write anything — a note, instructions, context for the dashboard…" style="margin-top:10px;min-height:120px;">${existing ? escHtml(existing.content) : ''}</textarea>
    <div class="row" style="margin-top:14px;">
      <button onclick="createTextboxWidget()">${isEdit ? 'Save changes' : 'Create'}</button>
      <button class="secondary" onclick="closeWidgetBuilder()">Cancel</button>
    </div>`;
  document.getElementById('widgetBuilder').innerHTML = html;
}
function createTextboxWidget() {
  const d = currentDashboard();
  if (!d) return;
  const props = { title: document.getElementById('tb-title').value || 'Note', content: document.getElementById('tb-content').value };
  snapshotWidgets();
  const sheet = currentSheet(d);
  if (builderState.editingId) {
    const w = sheet.widgets.find(w => w.id === builderState.editingId);
    ...
    sheet.widgets.push(newWidgetLayout({ id: Date.now(), kind: 'textbox', ...props }, sheet));
  }
  d.edited = fmtDate(new Date());
  closeWidgetBuilder();
  renderWidgets();
...
/* ---- Image (paste a URL, or upload a small image — stored as a data URL) ---- */
function renderImageConfig(existing) {
  const isEdit = !!(existing && builderState.editingId);
  const html = `
    <button class="link" onclick="${isEdit ? 'closeWidgetBuilder()' : 'openWidgetBuilder()'}">&larr; ${isEdit ? 'Close' : 'Back'}</button>
    <div style="margin-top:6px;"><b>${isEdit ? 'Edit: ' : ''}Image</b></div>
    <div class="row" style="margin-top:12px;"><input type="text" id="img-title" placeholder="Widget title (optional)" style="flex:1;" value="${esc(existing && existing.title)}"></div>
    <label style="font-size:12px;color:var(--text-dim);margin-top:10px;display:block;">Image URL</label>
    <input type="text" id="img-url" placeholder="https://…" value="${esc(existing && existing.url)}">
    <p class="subtitle" style="margin:8px 0 4px;text-align:center;">— or —</p>
    <label style="font-size:12px;color:var(--text-dim);">Upload a file (stored inline; keep it small — logos/icons, not photos)</label>
    <input type="file" id="img-file" accept="image/*" onchange="imgFileToDataURL(event)">
    <div id="imgPreviewWrap" style="margin-top:10px;${existing && existing.url ? '' : 'display:none;'}">
      <img id="imgPreview" src="${existing ? esc(existing.url) : ''}" style="max-width:100%;max-height:120px;border-radius:8px;">
    </div>
    <div class="row" style="margin-top:14px;">
      <button onclick="createImageWidget()">${isEdit ? 'Save changes' : 'Create'}</button>
      <button class="secondary" onclick="closeWidgetBuilder()">Cancel</button>
    </div>`;
  document.getElementById('widgetBuilder').innerHTML = html;
  document.getElementById('img-url').addEventListener('input', (e) => {
    document.getElementById('imgPreview').src = e.target.value;
    document.getElementById('imgPreviewWrap').style.display = e.target.value ? 'block' : 'none';
  });
}
function imgFileToDataURL(event) {
  const file = event.target.files[0];
  if (!file) return;
  if (file.size > 700 * 1024) { alert('Keep uploaded images under ~700KB — Firestore documents have a 1MB size limit and this dashboard may contain other widgets too.'); return; }
  const reader = new FileReader();
  reader.onload = () => {
    document.getElementById('img-url').value = reader.result;
...
function createImageWidget() {
  ...
  if (!url) { alert('Provide an image URL or upload a file first.'); return; }
  const props = { title: document.getElementById('img-title').value || 'Image', url };
  snapshotWidgets();
  const sheet = currentSheet(d);
  if (builderState.editingId) {
    const w = sheet.widgets.find(w => w.id === builderState.editingId);
    ...
    sheet.widgets.push(newWidgetLayout({ id: Date.now(), kind: 'image', ...props }, sheet));
  }
  d.edited = fmtDate(new Date());
  closeWidgetBuilder();
  renderWidgets();
...
function removeWidget(id) {
  ...
  snapshotWidgets();
  const sheet = currentSheet(d);
  sheet.widgets = sheet.widgets.filter(w => w.id !== id);
  renderWidgets();
  saveStateToStorage();
}
